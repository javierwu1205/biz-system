...
async function fireAdd(colName, data) { const { _id, ...c } = data; await addDoc(collection(db, colName), { ...c, _ts: Date.now() }); }
async function fireUpdate(colName, id, data) { const { _id, ...c } = data; await updateDoc(doc(db, colName, id), { ...c, _ts: Date.now() }); }
async function fireDelete(colName, id) { await deleteDoc(doc(db, colName, id)); }

// ─── ACCOUNTS (runtime-mutable via localStorage for password changes) ─────────
function getAccounts() {
  try {
    const saved = localStorage.getItem("biz_accounts");
...
function saveAccounts(accts) { try { localStorage.setItem("biz_accounts", JSON.stringify(accts)); } catch(e) {} }
...
const ALL_MEMBERS = ["Javier", "Admin", "Ryan", "Susan", "Chaymae", "Denny"];
const EMPLOYEE_NAMES = ["Ryan", "Susan", "Chaymae", "Denny"];
...
const STATUS_COLORS = { "待确认": "#f59e0b", "进行中": "#3b82f6", "已完成": "#10b981", "已取消": "#ef4444", "规划中": "#8b5cf6", "开发中": "#3b82f6", "测试中": "#f59e0b", "已上线": "#10b981", "暂停": "#6b7280", "潜在": "#f59e0b", "活跃": "#10b981", "沉睡": "#6b7280", "流失": "#ef4444" };
function Badge({ status }) { const c = STATUS_COLORS[status] || "#6b7280"; return <span style={{ background: c + "22", color: c, border: `1px solid ${c}44`, padding: "2px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600, whiteSpace: "nowrap" }}>{status}</span>; }
function Modal({ title, onClose, children, wide }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#1a1f2e", border: "1px solid #2d3748", borderRadius: 16, width: "100%", maxWidth: wide ? 700 : 560, maxHeight: "90vh", overflowY: "auto" }}>
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
        ...
        <tbody>
          {rows.length === 0
            ? <tr><td colSpan={headers.length + 1} style={{ textAlign: "center", padding: 40, color: "#4a5568" }}>暂无数据</td></tr>
            : rows.map((row, i) => (
...
// ─── EXPORT HELPERS ───────────────────────────────────────────────────────────
function exportToCSV(data, filename, cols) {
  const header = cols.join(",");
  const rows = data.map(r => cols.map(c => {
    const v = typeof r[c] === "object" ? "" : String(r[c] || "").replace(/,/g, "，").replace(/\n/g, " ");
    return `"${v}"`;
  }).join(","));
  const bom = "\uFEFF";
...
function exportToPDF(data, filename, cols, title) {
  const win = window.open("", "_blank");
  if (!win) return alert("请允许弹出窗口");
  const rows = data.map(r => `<tr>${cols.map(c => {
    const v = typeof r[c] === "object" ? "" : String(r[c] || "");
    return `<td>${v}</td>`;
  }).join("")}</tr>`).join("");
  win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${title}</title>
  <style>body{font-family:sans-serif;padding:20px}h2{color:#333}table{width:100%;border-collapse:collapse;font-size:12px}
  th{background:#1a1f2e;color:#fff;padding:8px;text-align:left}td{padding:7px;border-bottom:1px solid #ddd}
  tr:nth-child(even){background:#f9f9f9}@media print{body{padding:0}}</style></head>
  <body><h2>${title}</h2><p style="color:#666;font-size:12px">导出时间：${new Date().toLocaleString("zh-CN")}</p>
...
// ─── FILTER BAR ───────────────────────────────────────────────────────────────
function FilterBar({ isAdmin, filters, setFilters, showPerson = true }) {
  const allCountries = filters.region ? (COUNTRIES_BY_REGION[filters.region] || []) : [];
  return (
    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16, padding: "12px 16px", background: "#0f1420", borderRadius: 10, border: "1px solid #1e2433" }}>
      {isAdmin && showPerson && (
        <select style={{ ...SS, width: "auto", minWidth: 110 }} value={filters.person || ""} onChange={e => setFilters(p => ({ ...p, person: e.target.value }))}>
          <option value="">全部人员</option>
          {ALL_MEMBERS.map(m => <option key={m} value={m}>{m}</option>)}
        </select>
      )}
      <select style={{ ...SS, width: "auto", minWidth: 110 }} value={filters.region || ""} onChange={e => setFilters(p => ({ ...p, region: e.target.value, country: "" }))}>
        <option value="">全部地区</option>
        {REGIONS.map(r => <option key={r} value={r}>{r}</option>)}
      </select>
      {filters.region && (
        <select style={{ ...SS, width: "auto", minWidth: 120 }} value={filters.country || ""} onChange={e => setFilters(p => ({ ...p, country: e.target.value }))}>
          <option value="">全部国家</option>
          {allCountries.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
      )}
      <input style={{ ...IS, width: "auto", minWidth: 130 }} type="date" value={filters.dateFrom || ""} placeholder="开始日期" onChange={e => setFilters(p => ({ ...p, dateFrom: e.target.value }))} />
      ...
      {(filters.person || filters.region || filters.dateFrom || filters.dateTo) && (
        <Btn onClick={() => setFilters({})} style={{ background: "#2d3748", color: "#a0aec0", padding: "8px 14px", fontSize: 13 }}>✕ 清除筛选</Btn>
      )}
    </div>
  );
}

function applyFilters(data, filters, dateField = "日期") {
  return data.filter(d => {
    if (filters.person && d.负责人 !== filters.person && d._owner !== filters.person) return false;
    if (filters.region && d.地区 !== filters.region) return false;
    if (filters.country && d.国家 !== filters.country) return false;
...
function ChangePasswordModal({ user, onClose }) {
  ...
  function save() {
    ...
    const isAdminUser = !!accts.admins[user.username];
    const current = isAdminUser ? accts.admins[user.username].password : accts.members[user.username]?.password;
    if (oldPwd !== current) { setErr("旧密码不正确"); return; }
    if (newPwd.length < 6) { setErr("新密码至少 6 位"); return; }
    if (newPwd !== confirm) { setErr("两次密码不一致"); return; }
    if (isAdminUser) accts.admins[user.username].password = newPwd;
    ...
    <Modal title="🔐 修改密码" onClose={onClose}>
      {ok ? (
        <div style={{ textAlign: "center", padding: 20 }}>
          ...
          <div style={{ color: "#10b981", fontWeight: 600, fontSize: 16, marginBottom: 16 }}>密码修改成功！</div>
          <Btn onClick={onClose} style={{ background: "#2d3748", color: "#a0aec0", padding: "10px 24px" }}>关闭</Btn>
        </div>
      ) : (
        <>
          <Field label="旧密码"><input style={IS} type="password" value={oldPwd} onChange={e => setOldPwd(e.target.value)} /></Field>
          <Field label="新密码（至少 6 位）"><input style={IS} type="password" value={newPwd} onChange={e => setNewPwd(e.target.value)} /></Field>
          <Field label="确认新密码"><input style={IS} type="password" value={confirm} onChange={e => setConfirm(e.target.value)} /></Field>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Btn onClick={onClose} style={{ background: "#2d3748", color: "#a0aec0", padding: "10px 20px" }}>取消</Btn>
        ...
        <div style={{ background: "#1a1f2e", border: "1px solid #2d3748", borderRadius: 20, padding: 32 }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input style={{ ...IS, paddingLeft: 40 }} placeholder="输入用户名" value={username}
          ...
          <div style={{ marginBottom: err ? 8 : 24 }}>
            ...
            <div style={{ position: "relative" }}>
              <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", fontSize: 16 }}>🔑</span>
              <input style={{ ...IS, paddingLeft: 40, paddingRight: 44 }} type={show ? "text" : "password"} placeholder="输入密码"
...
function Orders({ data, setData, user }) {
  ...
  const exportCols = isAdmin
    ? ["客户名称", "地区", "国家", "金额", "成本", "货币", "负责人", "状态", "日期", "备注"]
    : ["客户名称", "地区", "国家", "金额", "成本", "货币", "状态", "日期", "备注"];
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ color: "#a0aec0", fontSize: 14 }}>{isAdmin ? <><b style={{ color: "#e2e8f0" }}>{filtered.length}</b> 条{filtered.length < data.length ? `（共${data.length}条）` : "（全部）"}</> : <><b style={{ color: "#e2e8f0" }}>{filtered.length}</b> 条我的订单</>}</div>
        <div style={{ display: "flex", gap: 8 }}>
          <Btn onClick={() => exportToCSV(filtered.map(d=>({...d,利润:Number(d.金额||0)-Number(d.成本||0)})), "销售订单_"+new Date().toLocaleDateString("zh-CN"), exportCols)} style={{ background: "#1e3a2e", color: "#10b981", padding: "8px 14px", fontSize: 13 }}>⬇ Excel</Btn>
          <Btn onClick={() => exportToPDF(filtered.map(d=>({...d,利润:Number(d.金额||0)-Number(d.成本||0)})), "销售订单", exportCols, "销售订单 — 煦彩")} style={{ background: "#1e2a3a", color: "#60a5fa", padding: "8px 14px", fontSize: 13 }}>⬇ PDF</Btn>
      ...
      <FilterBar isAdmin={isAdmin} filters={filters} setFilters={setFilters} />
...
function Development({ data, setData, user }) {
  ...
  const [filters, setFilters] = useState({});
  const filtered = applyFilters(visible, filters);
  const exportCols = ["客户名称", "地区", "国家", "负责人", "状态", "进度", "开始日期", "预计完成", "描述"];
  const headers = ["客户名称", "地区", "国家", "负责人", "状态", "进度", "开始日期", "预计完成"];
  const rows = filtered.map(d => ({ ...d, 进度: d.进度 + "%", _canEdit: d._owner === user.name }));
  const countries = COUNTRIES_BY_REGION[form.地区] || ["其他"];
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ color: "#a0aec0", fontSize: 14 }}>{isAdmin ? <><b style={{ color: "#e2e8f0" }}>{filtered.length}</b> 个{filtered.length < data.length ? `（共${data.length}个）` : "（全部）"}</> : <><b style={{ color: "#e2e8f0" }}>{filtered.length}</b> 个我的项目</>}</div>
        <div style={{ display: "flex", gap: 8 }}>
          <Btn onClick={() => exportToCSV(filtered.map(d=>({...d,进度:d.进度+"%"})), "开发进度_"+new Date().toLocaleDateString("zh-CN"), exportCols)} style={{ background: "#1e3a2e", color: "#10b981", padding: "8px 14px", fontSize: 13 }}>⬇ Excel</Btn>
          <Btn onClick={() => exportToPDF(filtered.map(d=>({...d,进度:d.进度+"%"})), "开发进度", exportCols, "开发进度 — 煦彩")} style={{ background: "#1e2a3a", color: "#60a5fa", padding: "8px 14px", fontSize: 13 }}>⬇ PDF</Btn>
...
function Clients({ data, setData, user }) {
  ...
  const fv = (k, v) => setForm(p => ({ ...p, [k]: v }));
  const [filters, setFilters] = useState({});
  const filtered = applyFilters(visible, filters, "最近联系");
  const exportCols = ["公司名称", "联系人", "邮箱", "地区", "国家", "状态", "负责人", "最近联系", "备注"];
  const headers = ["公司名称", "联系人", "邮箱", "地区", "国家", "状态", "负责人", "最近联系"];
  const rows = filtered.map(d => ({ ...d, _canEdit: d._owner === user.name }));
  const countries = COUNTRIES_BY_REGION[form.地区] || ["其他国家"];
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ color: "#a0aec0", fontSize: 14 }}>{isAdmin ? <><b style={{ color: "#e2e8f0" }}>{filtered.length}</b> 个{filtered.length < data.length ? `（共${data.length}个）` : "（全部）"}</> : <><b style={{ color: "#e2e8f0" }}>{filtered.length}</b> 个我的客户</>}</div>
        <div style={{ display: "flex", gap: 8 }}>
          <Btn onClick={() => exportToCSV(filtered, "客户管理_"+new Date().toLocaleDateString("zh-CN"), exportCols)} style={{ background: "#1e3a2e", color: "#10b981", padding: "8px 14px", fontSize: 13 }}>⬇ Excel</Btn>
          <Btn onClick={() => exportToPDF(filtered, "客户管理", exportCols, "客户管理 — 煦彩")} style={{ background: "#1e2a3a", color: "#60a5fa", padding: "8px 14px", fontSize: 13 }}>⬇ PDF</Btn>
...
function Reports({ data, setData, user }) {
  ...
  const [filters, setFilters] = useState({});
  const filteredReports = data.filter(d => {
    if (filters.person && d._owner !== filters.person && d.成员 !== filters.person) return false;
    if (filters.dateFrom && d.日期 && d.日期 < filters.dateFrom) return false;
    if (filters.dateTo && d.日期 && d.日期 > filters.dateTo) return false;
    return true;
  });
  const exportCols = ["成员", "类型", "日期", "本周完成", "下周计划", "遇到问题"];
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div style={{ color: "#a0aec0", fontSize: 14 }}><b style={{ color: "#e2e8f0" }}>{filteredReports.length}</b> 份汇报{filteredReports.length < data.length ? `（共${data.length}份）` : ""}</div>
        <div style={{ display: "flex", gap: 8 }}>
          {isAdmin && <><Btn onClick={() => exportToCSV(filteredReports, "工作汇报_"+new Date().toLocaleDateString("zh-CN"), exportCols)} style={{ background: "#1e3a2e", color: "#10b981", padding: "8px 14px", fontSize: 13 }}>⬇ Excel</Btn>
          <Btn onClick={() => exportToPDF(filteredReports, "工作汇报", exportCols, "工作汇报 — 煦彩")} style={{ background: "#1e2a3a", color: "#60a5fa", padding: "8px 14px", fontSize: 13 }}>⬇ PDF</Btn></>}
      ...
      {isAdmin && <FilterBar isAdmin={isAdmin} filters={filters} setFilters={setFilters} showPerson={true} />}
      <div style={{ color: "#4a5568", fontSize: 12, marginBottom: 20 }}>📢 汇报全员可见，仅可编辑自己的</div>
      <div style={{ display: "grid", gap: 16 }}>
        {filteredReports.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#4a5568" }}>暂无汇报</div>}
        {filteredReports.map((d, i) => (
...
export default function App() {
  ...
  const upOrders  = async (nd, a, i) => syncToFirestore("orders",  orders,  nd, a, i);
  const upDevs    = async (nd, a, i) => syncToFirestore("devs",    devs,    nd, a, i);
  const upClients = async (nd, a, i) => syncToFirestore("clients", clients, nd, a, i);
  const upReports = async (nd, a, i) => syncToFirestore("reports", reports, nd, a, i);
    ...
    <div style={{ minHeight: "100vh", background: "#0f1420", fontFamily: "'Noto Sans SC', system-ui, sans-serif" }}>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <img src="data:image/png;base64," alt="logo" style={{ height: 36, objectFit: "contain" }} />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <Btn onClick={() => setShowChangePwd(true)} style={{ background: "#1a2a1a", color: "#68d391", padding: "8px 12px", fontSize: 12 }}>🔐 改密码</Btn>
      ...
      {!isAdmin && (
        <div style={{ background: "#15101e", borderBottom: "1px solid #251c35" }}>
          <div style={{ maxWidth: 1400, margin: "0 auto", padding: "8px 24px", color: "#7c3aed", fontSize: 12 }}>
