...
function ThemeStyle({ T }) {
  ...
  const mobileCss = `
    @media (max-width: 768px) {
      /* ── NAVBAR ── */
      .nav-top { flex-wrap: wrap; height: auto !important; padding: 8px 12px !important; gap: 6px; }
      .nav-logo-area { gap: 6px !important; }
      .nav-logo-area img { height: 26px !important; }
      .nav-right { gap: 4px !important; flex-wrap: wrap; }
      .nav-user-badge { padding: 4px 8px !important; font-size: 12px !important; }
      .nav-btn { padding: 6px 8px !important; font-size: 11px !important; }
      /* Tab bar - horizontal scroll */
      .nav-tabs { display: flex; overflow-x: auto; -webkit-overflow-scrolling: touch; scrollbar-width: none; padding: 0 4px 4px; gap: 0 !important; }
      .nav-tabs::-webkit-scrollbar { display: none; }
      .nav-tab-btn { white-space: nowrap; padding: 8px 10px !important; font-size: 11px !important; flex-shrink: 0; }
      /* ── CONTENT ── */
      ...
      /* ── MODALS ── */
      .modal-box { width: 95vw !important; max-width: 95vw !important; padding: 16px !important; margin: 8px auto !important; max-height: 90vh; overflow-y: auto; }
      /* ── FILTER BAR ── */
      ...
      /* ── CALENDAR ── */
      .cal-grid { font-size: 11px !important; }
      ...
      /* ── BUTTONS ── */
      .action-btn-row { flex-wrap: wrap !important; gap: 6px !important; }
      /* ── AI CHAT ── */
      .ai-chat-window { width: 92vw !important; right: 4vw !important; bottom: 70px !important; }
      /* ── FOLLOW UP BANNER ── */
      .fu-banner { font-size: 12px !important; padding: 10px !important; }
      /* ── PIPELINE/TRACKING CARDS ── */
      .mobile-card { background: ${T.bg2}; border: 1px solid ${T.border}; border-radius: 12px; padding: 12px; margin-bottom: 10px; }
      .mobile-card-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px; }
      .mobile-card-title { color: ${T.text}; font-weight: 700; font-size: 14px; }
      .mobile-card-sub { color: ${T.text3}; font-size: 12px; margin-top: 2px; }
      .mobile-card-row { display: flex; justify-content: space-between; padding: 4px 0; border-bottom: 1px solid ${T.bg4}; }
      .mobile-card-label { color: ${T.text3}; font-size: 12px; }
      .mobile-card-value { color: ${T.text2}; font-size: 12px; font-weight: 600; }
      .mobile-card-actions { display: flex; gap: 8px; margin-top: 10px; flex-wrap: wrap; }
    }
    @media (max-width: 480px) {
      .stat-grid { grid-template-columns: 1fr !important; }
      .nav-right .nav-btn span { display: none; }
    }
  `;
  return <style dangerouslySetInnerHTML={{ __html: css + mobileCss }} />;
}
...
// ─── TRACKING 客户开发跟踪 ────────────────────────────────────────────────────
function Tracking({ T, isMobile, data, user, onAdd, onUpdate, onDelete }) {
...
// ─── PIPELINE 报价 & 销售机会池 ───────────────────────────────────────────────
function Pipeline({ T, isMobile, data, user, onAdd, onUpdate, onDelete, allClients, highlightId }) {
  const isSuper = user.role === "admin";
  const [modal, setModal] = useState(false);
    ...
    <div>
      ...
      {isMobile ? (
        <div className="mobile-card-list" style={{marginTop:10}}>
          {filtered.length===0 && <div style={{color:T.text3,textAlign:"center",padding:40}}>No data</div>}
          {filtered.map((r,i)=>(
            <div key={r._id} className="mobile-card" style={{background:T.bg2,border:`1px solid ${T.border}`,borderRadius:12,padding:14,marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div>
                  <div style={{color:T.text,fontWeight:700,fontSize:14}}>{r.Client}</div>
                  <div style={{color:T.text3,fontSize:12,marginTop:2}}>{r.Country} · {r._owner}</div>
                </div>
                <span style={{background:r.Status==="Won"?"#10b98122":r.Status==="Lost"?"#ef444422":"#f59e0b22",color:r.Status==="Won"?"#10b981":r.Status==="Lost"?"#ef4444":"#f59e0b",fontSize:11,padding:"3px 8px",borderRadius:20,fontWeight:700}}>{r.Status}</span>
              </div>
              {[["Product",r.Product],["Amount",r.Amount?`${r.Currency||"USD"} ${Number(r.Amount).toLocaleString()}`:"–"],["Date",r.Date],["Next Follow-up",r.FollowUp]].map(([k,v])=>v&&(
                <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:`1px solid ${T.bg4}`}}>
                  <span style={{color:T.text3,fontSize:12}}>{k}</span>
                  <span style={{color:T.text2,fontSize:12,fontWeight:600}}>{v}</span>
                </div>
              ))}
              {(isSuper||r._owner===user.name) && (
                <div style={{display:"flex",gap:8,marginTop:10}}>
                  <button onClick={()=>openModal(r)} style={{flex:1,background:T.accentBg,border:`1px solid ${T.accent}`,color:T.accent,borderRadius:8,padding:"7px 0",fontSize:12,cursor:"pointer"}}>✏️ Edit</button>
                  <button onClick={()=>del(r)} style={{flex:1,background:"#ef444422",border:"1px solid #ef4444",color:"#ef4444",borderRadius:8,padding:"7px 0",fontSize:12,cursor:"pointer"}}>🗑️ Delete</button>
                </div>
              )}
            </div>
          ))}
...
function ClientMgmt({ T, isMobile, data, user, onAdd, onUpdate, onDelete, followups, onAddFollowup }) {
  const isSuper = user.role === "admin";
  const [modal, setModal] = useState(false);
  const [editItem, setEditItem] = useState(null);
  const [filters, setFilters] = useState({});
...
// ─── REPORTS 工作汇报 ─────────────────────────────────────────────────────────
// ─── REPORTS 工作汇报 ─────────────────────────────────────────────────────────
function Reports({ T, isMobile, data, user, onAdd, onUpdate, onDelete }) {
  const isSuper = user.role === "admin";
  const [modal, setModal] = useState(false);
  const [editItem, setEditItem] = useState(null);
  const [filters, setFilters] = useState({});
...
function WeeklyActivity({ T, isMobile, pipeline, tracking, reports }) {
  const [filters, setFilters] = useState({});
...
function ClientHealth({ T, isMobile, pipeline, clients, user }) {
  const isSuper = user?.role === "admin";
...
function SalesDashboard({ T, isMobile, pipeline, tracking, clients, reports, goals, onGoalSave }) {
  const [filters, setFilters] = useState({});
...
function CalendarTodo({ T, isMobile, calTeam, calPersonal, calMemos, user, onAddTeam, onUpdateTeam, onDeleteTeam, onAddPersonal, onUpdatePersonal, onDeletePersonal, onSaveMemo }) {
  const isSuper = user?.role === "admin";
...
function TeamLeaderboard({ T, isMobile, pipeline, goals, user }) {
  const now = new Date();
    ...
    <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'Noto Sans SC',system-ui,sans-serif" }}>
      ...
      <div style={{ background:T.navBg, borderBottom:`1px solid ${T.navBorder}`, position:"sticky", top:0, zIndex:100 }}>
        <div style={{ padding:"0 12px" }}>
          <div className="nav-top" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:58 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <img src="data:image/png;base64," alt="logo" style={{ height:34, objectFit:"contain", background:"#fff", borderRadius:6, padding:"2px 4px" }} />
            ...
            <div className="nav-right" style={{ display:"flex", alignItems:"center", gap:8 }}>
              <div className="nav-user-badge" style={{ display:"flex", alignItems:"center", gap:8, background:T.bg2, border:`1px solid ${T.border}`, borderRadius:10, padding:"6px 12px" }}>
                ...
                {showThemePicker && (
                  <div style={{ position:"absolute", right:0, top:42, background:T.bg2, border:`1px solid ${T.border}`, borderRadius:12, padding:10, zIndex:999, minWidth:180, boxShadow:"0 8px 32px #00000066" }}>
                    ...
                    {Object.values(THEMES).map(th => (
                      <button key={th.key} onClick={()=>switchTheme(th.key)} style={{ display:"block", width:"100%", textAlign:"left", background:T.key===th.key?T.accentBg:"transparent", border:`1px solid ${T.key===th.key?T.accent:"transparent"}`, color:T.key===th.key?T.accent:T.text, borderRadius:8, padding:"7px 10px", cursor:"pointer", fontSize:13, marginBottom:4, fontWeight:T.key===th.key?700:400 }}>
                        {th.name}
                      </button>
                    ))}
                  </div>
              ...
              <button onClick={()=>{setUser(null);setTab(0);}} style={{ background:T.bg4, border:"none", color:T.text2, padding:"8px 14px", borderRadius:8, cursor:"pointer", fontSize:13 }}>Exit</button>
            </div>
          </div>
          <div className="nav-tabs" style={{ display:"flex", gap:2, overflowX:"auto" }}>
            {tabs.map((t,i) => (
              <button key={t.key} className="nav-tab-btn" onClick={()=>setTab(i)} style={{ background:"none", border:"none", cursor:"pointer", padding:"10px 14px", fontSize:12, fontWeight:tab===i?700:400, color:tab===i?T.accent:T.text3, borderBottom:tab===i?`2px solid ${T.accent}`:"2px solid transparent", whiteSpace:"nowrap", transition:"all 0.2s" }}>
                {t.icon} {t.label}
              </button>
            ))}
          </div>
      ...
      {/* CONTENT */}
      <div className="page-content" style={{ padding:"20px 24px", background:T.bg, minHeight:"calc(100vh - 120px)" }}>
        ...
        {curTab==="dashboard" && isSuper && <SalesDashboard T={T} isMobile={isMobile} pipeline={pipeline} tracking={tracking} clients={clients} reports={reports} goals={goals} onGoalSave={(id,d)=>id?fireUpdate("goals",id,d):fireAdd("goals",d)} />}
        {curTab==="pipeline"  && <Pipeline  T={T} isMobile={isMobile} data={pipeline} user={user} onAdd={d=>op("pipeline","add",d)} onUpdate={(id,d)=>op("pipeline","update",{...d,_id:id})} onDelete={id=>fireDelete("pipeline",id)} allClients={clients} highlightId={highlightId} />}
        {curTab==="tracking"  && <Tracking  T={T} isMobile={isMobile} data={tracking} user={user} onAdd={d=>op("tracking","add",d)} onUpdate={(id,d)=>op("tracking","update",{...d,_id:id})} onDelete={id=>fireDelete("tracking",id)} />}
        {curTab==="clients"   && <ClientMgmt T={T} isMobile={isMobile} data={clients} user={user} onAdd={d=>op("clients2","add",d)} onUpdate={(id,d)=>op("clients2","update",{...d,_id:id})} onDelete={id=>fireDelete("clients2",id)} followups={followups} onAddFollowup={d=>fireAdd("followups",d)} />}
        {curTab==="reports"   && <Reports   T={T} isMobile={isMobile} data={reports} user={user} onAdd={d=>op("reports","add",d)} onUpdate={(id,d)=>op("reports","update",{...d,_id:id})} onDelete={id=>fireDelete("reports",id)} />}
        {curTab==="calendar"    && <CalendarTodo T={T} isMobile={isMobile} calTeam={calTeam} calPersonal={calPersonal} calMemos={calMemos} user={user} onAddTeam={d=>fireAdd("cal_team",d)} onUpdateTeam={(id,d)=>fireUpdate("cal_team",id,d)} onDeleteTeam={id=>fireDelete("cal_team",id)} onAddPersonal={d=>fireAdd("cal_personal",d)} onUpdatePersonal={(id,d)=>fireUpdate("cal_personal",id,d)} onDeletePersonal={id=>fireDelete("cal_personal",id)} onSaveMemo={(id,d)=>id?fireUpdate("cal_memos",id,d):fireAdd("cal_memos",d)} />}
        {curTab==="activity"  && isSuper && <WeeklyActivity T={T} isMobile={isMobile} pipeline={pipeline} tracking={tracking} reports={reports} />}
        {curTab==="health"    && <ClientHealth T={T} isMobile={isMobile} pipeline={pipeline} clients={clients} user={user} />}
        {curTab==="leaderboard" && <TeamLeaderboard T={T} isMobile={isMobile} pipeline={pipeline} goals={goals} user={user} />}
      </div>
