...
const THEMES = {
  dark: {
    name:"🌑 深色 Dark", key:"dark",
    bg:"#0f1420", bg2:"#1a1f2e", bg3:"#0a0e17", bg4:"#161b27",
    border:"#2d3748", border2:"#1e2433",
    text:"#e2e8f0", text2:"#a0aec0", text3:"#718096", text4:"#4a5568",
    accent:"#a78bfa", accentBg:"#a78bfa22",
    navBg:"#0a0e17", navBorder:"#1e2433",
    cardBg:"#1a1f2e", rowHover:"#1e2433",
    inputBg:"#0f1420",
  },
  light: {
  ...
  ocean: {
    ...
    bg:"#0a1628", bg2:"#0f2040", bg3:"#071020", bg4:"#0d1a35",
    border:"#1a3a5c", border2:"#0f2a48",
    text:"#e0f0ff", text2:"#90c0e8", text3:"#5090c0", text4:"#2a5a88",
    accent:"#38bdf8", accentBg:"#38bdf818",
    navBg:"#071020", navBorder:"#0f2a48",
    cardBg:"#0f2040", rowHover:"#132844",
    inputBg:"#0a1628",
  },
  forest: {
    name:"🌿 森绿 Forest", key:"forest",
    bg:"#0d1f0f", bg2:"#122414", bg3:"#091509", bg4:"#101c10",
    border:"#1e3d20", border2:"#162c18",
    text:"#d4edd6", text2:"#8cbd90", text3:"#5a8c5e", text4:"#2e5030",
    accent:"#4ade80", accentBg:"#4ade8018",
    navBg:"#091509", navBorder:"#162c18",
    cardBg:"#122414", rowHover:"#162c18",
    inputBg:"#0d1f0f",
  },
};
function getTheme() {
  try { const k = localStorage.getItem("biz_theme"); if (k && THEMES[k]) return THEMES[k]; } catch(e) {}
  return THEMES.dark;
}
  ...
  return (
    <div style={{ background:bg, border:`1px solid #f59e0b55`, borderRadius:12, padding:"14px 18px", marginBottom:20 }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
        ...
        <span style={{ color:"#a0aec0", fontSize:11, marginLeft:4 }}>— 点击客户名称直接跳转 Click to jump</span>
      </div>
      <div style={{ display:"grid", gap:6 }}>
        {due.map((d,i) => (
          <div key={i}
            onClick={() => onJump && onJump(d._id)}
            style={{ display:"flex", alignItems:"center", gap:10, background:"#00000033", borderRadius:8, padding:"8px 12px", cursor:onJump?"pointer":"default", transition:"all 0.15s" }}
            onMouseEnter={e=>{ if(onJump) e.currentTarget.style.background="#f59e0b22"; }}
            onMouseLeave={e=>{ e.currentTarget.style.background="#00000033"; }}>
            <span style={{ fontSize:14 }}>⚠️</span>
            <span style={{ color:"#fcd34d", fontWeight:700, fontSize:13, textDecoration:onJump?"underline":"none" }}>{d.Client}</span>
            <span style={{ color:"#a0aec0", fontSize:12 }}>·</span>
            <span style={{ color:"#a0aec0", fontSize:12 }}>{d.Stage||"—"}</span>
            <span style={{ color:"#a0aec0", fontSize:12 }}>·</span>
...
function LoginScreen({ onLogin }) {
  ...
  function login() {
    ...
    setTimeout(() => {
      ...
      if (accts.admins[u]) {
        if (pwd === accts.admins[u].password) { onLogin({ role:"admin", name:accts.admins[u].name, username:u }); return; }
        setErr("用户名或密码错误"); setLoading(false); return;
      }
      if (accts.members[u]) {
        if (pwd === accts.members[u].password) { onLogin({ role:"member", name:accts.members[u].name, username:u }); return; }
        setErr("用户名或密码错误"); setLoading(false); return;
      }
      setErr("用户名不存在"); setLoading(false);
    }, 400);
  }

  const LOGO = "data:image/png;base64,";

  return (
    <div style={{ minHeight:"100dvh", background:"#0f1420", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"24px 20px", boxSizing:"border-box" }}>
      <div style={{ width:"100%", maxWidth:420 }}>
        {/* Logo + Title */}
        <div style={{ textAlign:"center", marginBottom:36 }}>
          <div style={{ width:80, height:80, background:"#fff", borderRadius:20, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 18px", boxShadow:"0 8px 32px #667eea44" }}>
            <span style={{ fontSize:40 }}>🎨</span>
          </div>
          <h1 style={{ color:"#e2e8f0", fontSize:24, fontWeight:800, margin:"0 0 8px", letterSpacing:"-0.5px" }}>煦彩国际</h1>
          <p style={{ color:"#667eea", fontSize:14, margin:0, fontWeight:600 }}>业务管理系统 · 手机版</p>
        </div>

        {/* Login Card */}
        <div style={{ background:"#1a1f2e", border:"1px solid #2d3748", borderRadius:24, padding:28, boxShadow:"0 20px 60px #00000066" }}>
          <div style={{ marginBottom:18 }}>
            <label style={{ display:"block", color:"#a0aec0", fontSize:12, marginBottom:8, fontWeight:600, letterSpacing:"0.5px", textTransform:"uppercase" }}>用户名 Username</label>
            <input
              style={{ width:"100%", background:"#0f1420", border:"1px solid #2d3748", borderRadius:12, color:"#e2e8f0", padding:"15px 16px", fontSize:16, outline:"none", boxSizing:"border-box", WebkitAppearance:"none", transition:"border-color 0.2s" }}
              placeholder="请输入用户名"
              value={username}
          ...
          <div style={{ marginBottom:22 }}>
            <label style={{ display:"block", color:"#a0aec0", fontSize:12, marginBottom:8, fontWeight:600, letterSpacing:"0.5px", textTransform:"uppercase" }}>密码 Password</label>
            <div style={{ position:"relative" }}>
              <input
                style={{ width:"100%", background:"#0f1420", border:"1px solid #2d3748", borderRadius:12, color:"#e2e8f0", padding:"15px 50px 15px 16px", fontSize:16, outline:"none", boxSizing:"border-box", WebkitAppearance:"none" }}
                ...
                onKeyDown={e=>e.key==="Enter"&&login()}
              />
              <button onClick={()=>setShow(v=>!v)} style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", fontSize:22, color:"#718096", padding:4, lineHeight:1 }}>
                {show?"🙈":"👁️"}
              </button>
            </div>
          </div>
          ...
          {err && (
            <div style={{ color:"#fc8181", fontSize:13, marginBottom:16, textAlign:"center", background:"#3d1515", border:"1px solid #ef444433", padding:"10px 14px", borderRadius:10 }}>
              ⚠️ {err}
            </div>
          )}
          ...
          <button
            onClick={login}
            disabled={loading}
            style={{ width:"100%", background:loading?"#4a3f6b":"linear-gradient(135deg,#667eea,#764ba2)", border:"none", color:"#fff", padding:17, borderRadius:14, cursor:loading?"not-allowed":"pointer", fontWeight:700, fontSize:17, letterSpacing:"0.3px", boxShadow:loading?"none":"0 4px 20px #667eea44", transition:"all 0.2s" }}
          >
            {loading ? "登录中..." : "登 录"}
          </button>
        </div>

        <p style={{ textAlign:"center", color:"#4a5568", fontSize:12, marginTop:24 }}>Flowcolour International · 煦彩国际</p>
      </div>
    </div>
  );
}
...
function App() {
  ...
  function switchTheme(key) { const t=THEMES[key]; setTheme(t); saveTheme(key); }

  function jumpToPipeline(itemId) {
    const idx = tabs.findIndex(t => t.key === "pipeline");
    if (idx >= 0) { setTab(idx); setHighlightId(itemId); }
  ...
  const [goals,     goalLoaded]  = useFireCollection("goals");
  const [followups]              = useFireCollection("followups");
  const [calTeam]                = useFireCollection("cal_team");
  const [calPersonal]            = useFireCollection("cal_personal");
  const [calMemos]               = useFireCollection("cal_memos");

  const [forceLoaded, setForceLoaded] = useState(false);
  useEffect(() => { const t = setTimeout(() => setForceLoaded(true), 4000); return () => clearTimeout(t); }, []);
  const loaded = forceLoaded || (pipeLoaded && trackLoaded && cliLoaded && repLoaded && goalLoaded);

  const isSuper = user?.role === "admin";

  // ── Tab definitions ──────────────────────────────────────
  // Members: 5 main + health + leaderboard → use "more" tab
  // Admins:  calendar + dashboard + pipeline + tracking + clients + more
  const MEMBER_TABS = [
    { key:"calendar",    label:"待办",     icon:"📅" },
    { key:"pipeline",    label:"Pipeline", icon:"📦" },
    { key:"tracking",    label:"开发",     icon:"🎯" },
    { key:"clients",     label:"客户",     icon:"👥" },
    { key:"more",        label:"更多",     icon:"☰"  },
  ];
  const ADMIN_TABS = [
    { key:"calendar",    label:"待办",     icon:"📅" },
    { key:"dashboard",   label:"总监",     icon:"📊" },
    { key:"pipeline",    label:"Pipeline", icon:"📦" },
    { key:"clients",     label:"客户",     icon:"👥" },
    { key:"more",        label:"更多",     icon:"☰"  },
  ];
  const tabs = isSuper ? ADMIN_TABS : MEMBER_TABS;
  const curTab = tabs[tab]?.key || "calendar";

  // "More" subtabs differ by role
  const MEMBER_MORE = [
    { key:"reports",     label:"📝 工作汇报" },
    { key:"tracking",    label:"🎯 客户开发" },
    { key:"health",      label:"❤️ 客户健康度" },
    { key:"leaderboard", label:"🏆 业绩目标" },
  ];
  const ADMIN_MORE = [
    { key:"tracking",    label:"🎯 客户开发" },
    { key:"reports",     label:"📝 工作汇报" },
    { key:"activity",    label:"📈 行为管理" },
    { key:"health",      label:"❤️ 客户健康度" },
    { key:"leaderboard", label:"🏆 业绩目标" },
  ];
  const moreSubtabs = isSuper ? ADMIN_MORE : MEMBER_MORE;
  const [moreSub, setMoreSub] = useState(moreSubtabs[0].key);
  ...
  async function op(col, action, item) {
    ...
    if (col==="pipeline" && (action==="add"||action==="update")) {
      const latestDate = item.Date || "";
      if (latestDate && item.Client) {
        const mc = clients.find(c=>(c.Client||c.公司名称)===item.Client&&(c.Sales===item.Sales||c._owner===item._owner||c.Sales===item._owner))
                || clients.find(c=>(c.Client||c.公司名称)===item.Client);
        if (mc && latestDate>(mc.LastContact||"")) await fireUpdate("clients2",mc._id,{...mc,LastContact:latestDate});
      }
    }
  }
  ...
  if (!loaded) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100dvh", background:T.bg, flexDirection:"column", gap:16 }}>
      <div style={{ fontSize:48 }}>⏳</div>
      <div style={{ color:T.text3, fontSize:15 }}>Loading data...</div>
    </div>
  );

  // ── Render active tab content ─────────────────────────────
  function renderContent() {
    if (curTab==="calendar") return (
      <CalendarTodo T={T} calTeam={calTeam} calPersonal={calPersonal} calMemos={calMemos} user={user}
        onAddTeam={d=>fireAdd("cal_team",d)} onUpdateTeam={(id,d)=>fireUpdate("cal_team",id,d)} onDeleteTeam={id=>fireDelete("cal_team",id)}
        onAddPersonal={d=>fireAdd("cal_personal",d)} onUpdatePersonal={(id,d)=>fireUpdate("cal_personal",id,d)} onDeletePersonal={id=>fireDelete("cal_personal",id)}
        onSaveMemo={(id,d)=>id?fireUpdate("cal_memos",id,d):fireAdd("cal_memos",d)} />
    );
    if (curTab==="dashboard" && isSuper) return (
      <SalesDashboard T={T} pipeline={pipeline} tracking={tracking} clients={clients} reports={reports} goals={goals}
        onGoalSave={(id,d)=>id?fireUpdate("goals",id,d):fireAdd("goals",d)} />
    );
    if (curTab==="pipeline") return (
      <Pipeline T={T} data={pipeline} user={user}
        onAdd={d=>op("pipeline","add",d)} onUpdate={(id,d)=>op("pipeline","update",{...d,_id:id})}
        onDelete={id=>fireDelete("pipeline",id)} allClients={clients} highlightId={highlightId} />
    );
    if (curTab==="tracking") return (
      <Tracking T={T} data={tracking} user={user}
        onAdd={d=>op("tracking","add",d)} onUpdate={(id,d)=>op("tracking","update",{...d,_id:id})}
        onDelete={id=>fireDelete("tracking",id)} />
    );
    if (curTab==="clients") return (
      <ClientMgmt T={T} data={clients} user={user}
        onAdd={d=>op("clients2","add",d)} onUpdate={(id,d)=>op("clients2","update",{...d,_id:id})}
        onDelete={id=>fireDelete("clients2",id)} followups={followups} onAddFollowup={d=>fireAdd("followups",d)} />
    );
    if (curTab==="more") return (
      <div>
        ...
        <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:4, marginBottom:16, WebkitOverflowScrolling:"touch" }}>
          {moreSubtabs.map(s=>(
            <button key={s.key} onClick={()=>setMoreSub(s.key)}
              style={{ flexShrink:0, whiteSpace:"nowrap", background:moreSub===s.key?T.accentBg:T.bg2, border:`1px solid ${moreSub===s.key?T.accent:T.border}`, color:moreSub===s.key?T.accent:T.text2, borderRadius:20, padding:"9px 16px", cursor:"pointer", fontSize:13, fontWeight:moreSub===s.key?700:400, transition:"all 0.15s" }}>
              {s.label}
            </button>
          ))}
        </div>
        {moreSub==="reports"     && <Reports T={T} data={reports} user={user} onAdd={d=>op("reports","add",d)} onUpdate={(id,d)=>op("reports","update",{...d,_id:id})} onDelete={id=>fireDelete("reports",id)} />}
        {moreSub==="tracking"    && <Tracking T={T} data={tracking} user={user} onAdd={d=>op("tracking","add",d)} onUpdate={(id,d)=>op("tracking","update",{...d,_id:id})} onDelete={id=>fireDelete("tracking",id)} />}
        {moreSub==="activity"    && isSuper && <WeeklyActivity T={T} pipeline={pipeline} tracking={tracking} reports={reports} />}
        {moreSub==="health"      && <ClientHealth T={T} pipeline={pipeline} clients={clients} user={user} />}
        {moreSub==="leaderboard" && <TeamLeaderboard T={T} pipeline={pipeline} goals={goals} user={user} />}
      </div>
    );
    return null;
  }
  ...
  return (
    <div style={{ minHeight:"100dvh", background:T.bg, fontFamily:"'Noto Sans SC',system-ui,sans-serif", paddingBottom:NAV_H }}>
      <ThemeStyle T={T} />

      {/* Modals */}
      {showRates && isSuper && <ExchangeRateSettings onClose={()=>setShowRates(false)} user={user} />}
      {showPwd && <ChangePasswordModal user={user} onClose={()=>setShowPwd(false)} />}

      {/* ── TOP HEADER ─────────────────────────────────── */}
      <div style={{ position:"sticky", top:0, zIndex:200, background:T.navBg, borderBottom:`1px solid ${T.navBorder}`, height:HEAD_H, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 14px", boxShadow:"0 1px 12px #00000033" }}>
        {/* Left: logo + title */}
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ background:"#fff", borderRadius:8, padding:"3px 6px", display:"flex", alignItems:"center" }}>
            <span style={{ fontSize:20 }}>🎨</span>
          </div>
          <div style={{ lineHeight:1.2 }}>
            <div style={{ color:T.text, fontWeight:800, fontSize:14 }}>煦彩国际</div>
            <div style={{ color:T.text3, fontSize:10 }}>📱 Mobile {isSuper?"· 管理员":""}</div>
          </div>
        </div>

        {/* Right: user badge + menu */}
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ background:T.bg2, border:`1px solid ${T.border}`, borderRadius:16, padding:"4px 10px", display:"flex", alignItems:"center", gap:5 }}>
            <span style={{ fontSize:13 }}>{isSuper?"👑":"👤"}</span>
            <span style={{ color:T.text, fontSize:13, fontWeight:700 }}>{user?.name}</span>
          </div>
          <button onClick={()=>setShowMenu(p=>!p)}
            style={{ background:T.bg2, border:`1px solid ${T.border}`, borderRadius:10, width:38, height:38, cursor:"pointer", fontSize:18, color:T.text, display:"flex", alignItems:"center", justifyContent:"center" }}>
            {showMenu?"✕":"☰"}
          </button>
        </div>
      </div>
      ...
      {showMenu && (
        <>
          <div onClick={()=>setShowMenu(false)} style={{ position:"fixed", inset:0, zIndex:299, background:"#00000044" }} />
          <div style={{ position:"fixed", top:HEAD_H, left:0, right:0, zIndex:300, background:T.bg2, borderBottom:`2px solid ${T.border}`, padding:16, boxShadow:"0 12px 40px #00000088" }}>
            {/* Action buttons */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
              <button onClick={()=>{ setShowPwd(true); setShowMenu(false); }}
                style={{ background:T.bg3, border:`1px solid ${T.border}`, color:T.accent, borderRadius:12, padding:"13px 8px", cursor:"pointer", fontSize:13, fontWeight:700 }}>
                🔐 修改密码
              </button>
              {isSuper && <button onClick={()=>{ setShowRates(true); setShowMenu(false); }}
                style={{ background:T.bg3, border:`1px solid ${T.border}`, color:"#f59e0b", borderRadius:12, padding:"13px 8px", cursor:"pointer", fontSize:13, fontWeight:700 }}>
                💱 汇率设置
              </button>}
              {isSuper && <button onClick={()=>{ exportFullBackup(pipeline,tracking,clients,reports,followups,calTeam,calPersonal,calMemos); setShowMenu(false); }}
                style={{ background:T.bg3, border:`1px solid ${T.border}`, color:"#10b981", borderRadius:12, padding:"13px 8px", cursor:"pointer", fontSize:13, fontWeight:700 }}>
                💾 数据备份
              </button>}
              <button onClick={()=>{ setUser(null); setTab(0); setShowMenu(false); }}
                style={{ background:"#3d1515", border:"1px solid #ef444444", color:"#ef4444", borderRadius:12, padding:"13px 8px", cursor:"pointer", fontSize:13, fontWeight:700 }}>
                🚪 退出登录
              </button>
            </div>
            {/* Theme picker */}
            <div style={{ color:T.text3, fontSize:11, fontWeight:700, marginBottom:8, textTransform:"uppercase", letterSpacing:"0.5px" }}>🎨 主题 Theme</div>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              {Object.values(THEMES).map(th=>(
                <button key={th.key} onClick={()=>{ switchTheme(th.key); setShowMenu(false); }}
                  style={{ background:T.key===th.key?T.accentBg:T.bg3, border:`1px solid ${T.key===th.key?T.accent:T.border}`, color:T.key===th.key?T.accent:T.text, borderRadius:10, padding:"8px 14px", cursor:"pointer", fontSize:13, fontWeight:T.key===th.key?700:400 }}>
                  {th.name}
                </button>
              ))}
            </div>
      ...
      {/* ── MAIN CONTENT ───────────────────────────────── */}
      <div style={{ padding:"14px 12px", minHeight:`calc(100dvh - ${HEAD_H+NAV_H}px)` }}>
        {curTab!=="pipeline" && <FollowUpBanner pipeline={pipeline} user={user} onJump={jumpToPipeline} T={T} />}
        {renderContent()}
      </div>

      {/* ── BOTTOM TAB BAR ─────────────────────────────── */}
      <div style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:200, background:T.navBg, borderTop:`2px solid ${T.navBorder}`, display:"flex", height:NAV_H, paddingBottom:"env(safe-area-inset-bottom)", boxShadow:"0 -4px 20px #00000044" }}>
        {tabs.map((t,i)=>(
          <button key={t.key} onClick={()=>{ setTab(i); setShowMenu(false); }}
            style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"none", border:"none", cursor:"pointer", padding:"6px 2px", gap:3, position:"relative", transition:"all 0.15s" }}>
            {/* Active indicator bar */}
            {tab===i && <div style={{ position:"absolute", top:0, left:"20%", right:"20%", height:2, background:T.accent, borderRadius:"0 0 4px 4px" }} />}
            <span style={{ fontSize:tab===i?26:22, lineHeight:1, transition:"all 0.15s", transform:tab===i?"translateY(-2px) scale(1.1)":"scale(1)", filter:tab===i?"none":"opacity(0.65)" }}>{t.icon}</span>
            <span style={{ fontSize:10, fontWeight:tab===i?700:500, color:tab===i?T.accent:T.text3, transition:"all 0.15s" }}>{t.label}</span>
          </button>
        ))}
      </div>
