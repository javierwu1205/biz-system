...
function Pipeline({ T, data=[], user, onAdd, onUpdate, onDelete, allClients=[], highlightId }) {
  ...
  return (
    <div>
      ...
      <FollowUpBanner pipeline={data} user={user} onJump={id => setLocalHighlightId(id)} T={T} />
      ...
      <FilterBar isSuper={isSuper} filters={filters} setFilters={setFilters} />
      <SortableTable headers={headers} rows={rows} onEdit={i=>openModal(filtered[i])} onDelete={del} canEdit={r=>isSuper||r._owner===user?.name} defaultSort="Date" sortDesc={true} sortKeyMap={sortKeyMap} highlightId={highlightId || localHighlightId} />
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'Noto Sans SC',system-ui,sans-serif" }}>
      ...
      <div style={{ background:T.navBg, borderBottom:`1px solid ${T.navBorder}`, position:"sticky", top:0, zIndex:100 }}>
        <div style={{ padding:"0 24px" }}>
          ...
          <div style={{ display:"flex", gap:2, overflowX:"auto" }}>
            {tabs.map((t,i) => (
              <button key={t.key} onClick={()=>setTab(i)} style={{ background:"none", border:"none", cursor:"pointer", padding:"10px 14px", fontSize:12, fontWeight:tab===i?700:400, color:tab===i?T.accent:T.text3, borderBottom:tab===i?`2px solid ${T.accent}`:"2px solid transparent", whiteSpace:"nowrap", transition:"all 0.2s" }}>
                {t.icon} {t.label}
              </button>
            ))}
          </div>
      ...
      <div style={{ padding:"20px 24px", background:T.bg, minHeight:"calc(100vh - 120px)" }}>
        ...
        {curTab==="dashboard" && isSuper && <SalesDashboard T={T} pipeline={pipeline} tracking={tracking} clients={clients} reports={reports} goals={goals} onGoalSave={(id,d)=>id?fireUpdate("goals",id,d):fireAdd("goals",d)} />}
        {curTab==="pipeline"  && <Pipeline  T={T} data={pipeline} user={user} onAdd={d=>op("pipeline","add",d)} onUpdate={(id,d)=>op("pipeline","update",{...d,_id:id})} onDelete={id=>fireDelete("pipeline",id)} allClients={clients} highlightId={highlightId} />}
        {curTab==="tracking"  && <Tracking  T={T} data={tracking} user={user} onAdd={d=>op("tracking","add",d)} onUpdate={(id,d)=>op("tracking","update",{...d,_id:id})} onDelete={id=>fireDelete("tracking",id)} />}
        {curTab==="clients"   && <ClientMgmt T={T} data={clients} user={user} onAdd={d=>op("clients2","add",d)} onUpdate={(id,d)=>op("clients2","update",{...d,_id:id})} onDelete={id=>fireDelete("clients2",id)} followups={followups} onAddFollowup={d=>fireAdd("followups",d)} />}
        {curTab==="reports"   && <Reports   T={T} data={reports} user={user} onAdd={d=>op("reports","add",d)} onUpdate={(id,d)=>op("reports","update",{...d,_id:id})} onDelete={id=>fireDelete("reports",id)} />}
        {curTab==="calendar"    && <CalendarTodo T={T} calTeam={calTeam} calPersonal={calPersonal} calMemos={calMemos} user={user} onAddTeam={d=>fireAdd("cal_team",d)} onUpdateTeam={(id,d)=>fireUpdate("cal_team",id,d)} onDeleteTeam={id=>fireDelete("cal_team",id)} onAddPersonal={d=>fireAdd("cal_personal",d)} onUpdatePersonal={(id,d)=>fireUpdate("cal_personal",id,d)} onDeletePersonal={id=>fireDelete("cal_personal",id)} onSaveMemo={(id,d)=>id?fireUpdate("cal_memos",id,d):fireAdd("cal_memos",d)} />}
        {curTab==="activity"  && isSuper && <WeeklyActivity T={T} pipeline={pipeline} tracking={tracking} reports={reports} />}
        {curTab==="health"    && <ClientHealth T={T} pipeline={pipeline} clients={clients} user={user} />}
        {curTab==="leaderboard" && <TeamLeaderboard T={T} pipeline={pipeline} goals={goals} user={user} />}
      </div>
