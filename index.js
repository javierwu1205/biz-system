...
// ─── PIPELINE 报价 & 销售机会池 ───────────────────────────────────────────────
function Pipeline({ data, user, onAdd, onUpdate, onDelete, allClients, highlightId }) {
  const isSuper = user.role === "admin";
  const [modal, setModal] = useState(false);
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'Noto Sans SC',system-ui,sans-serif" }}>
      ...
      <div style={{ padding:"20px 24px", background:T.bg, minHeight:"calc(100vh - 120px)" }}>
        ...
        {curTab==="pipeline"  && <Pipeline  data={pipeline} user={user} onAdd={d=>op("pipeline","add",d)} onUpdate={(id,d)=>op("pipeline","update",{...d,_id:id})} onDelete={id=>fireDelete("pipeline",id)} allClients={clients} highlightId={highlightId} />}
